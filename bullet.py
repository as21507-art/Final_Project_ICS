...
class Bullet:
	...
	def __init__(self, player, tank_x, tank_y, tank_radius, velocity, angle, mode = "default"):
		...
		self.direction = math.pow(-1, player + 1)						# For player 1, X values are incremented (+1) and for player 2, X values are decremented (-1)
	...
	def move(self):
		if not self.landed:
			if frameCount % 30 == 0:									# Slows down the animation of bullet
